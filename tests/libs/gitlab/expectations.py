...
class GitLabMergeRequestNoteExpectation(GitLabExpectation):
    def __init__(self, project_id: int, merge_request_iid: int, discussion_id: str, note: str) -> None:
    ...
    def matches(self, project_id: int, merge_request_iid: int, discussion_id: str, note: str) -> bool:
        return self.project_id == project_id and \
               self.merge_request_iid == merge_request_iid and \
               self.discussion_id == discussion_id and \
               self.note == note
...
class GitLabExpectations:
    ...
    def merge_request_note_created(
            self, project_id: int, merge_request_iid: int, discussion_id: str, note: str
    ) -> None:
        self.merge_request_notes.append(
            GitLabMergeRequestNoteExpectation(project_id, merge_request_iid, discussion_id, note))
